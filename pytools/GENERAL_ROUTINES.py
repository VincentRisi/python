...
def printClass(c, name='top', level=0):
   pStr = '';
   if name == 'top':
      if hasattr(c, 'name'):
         name = c.name

      pStr = f'printClass({name})' + '\n{'
   ...
   for item in items:
       try:
           ...
           if hasattr(attr, '__dict__') or hasattr(attr, '__slots__'):
               pStr += printClass(attr, '%s.%s' % (name, item), level = 1)
           else:
...
# -- This printClass has no eval -- but uses standard attr routines
def classToDict(c, addEmptField = False, name='') -> {}:
   ...
   for item in items:
       try:
           attr = getattr(c, item)
           if hasattr(attr, '__dict__') or hasattr(attr, '__slots__'):
               classToDict(attr, '%s.%s' % (name, item))
           else:
              dicValue = '%s' % str(attr)
              if addEmptField and (dicValue == "''" or dicValue ==  "{}" or len(dicValue) == 0):
                 continue
              dicName = '%s%s' % (name, item)
              dic[dicName] = dicValue
       except Exception as ex:
           print (name, item, ex)
   return dic

def PrintFileLine(PreeFixStr='', cf = currentframe(), aLogFunction = None):
   sf1 = sys._getframe(1)
   function = sf1.f_code.co_name
   lineno = sf1.f_lineno
   file = sf1.f_code.co_filename
   Jotter = ('%s %s %s Line %d' % (PreeFixStr, function, file, lineno))
   if aLogFunction != None:
      aLogFunction(Jotter)
   return

def Catch(ModuleName, xType, xValue, xTraceback, aPrint=True):
   sf = sys._getframe(1)
   location = 'Exception caught At ' + sf.f_code.co_filename.split('\\\\')[-1] + '(' + str(sf.f_lineno) + ')->' + sys._getframe(1).f_code.co_name
   if type(xValue) is tuple:
      RaiseMsg = str(xValue)
      Jotter = "\nException thrown from Module: " + ModuleName + \
...
def _build_json(classes_dict, attribs_dict, group_name, done):
    ...
    for field_name, field_value in classes_dict[group_name]:
        field_value = field_value.replace("'", "&squot")
...
def dict_to_json(input_dict):
    ...
    json_text = json.dumps(build)
    json_text = json_text.replace("&squot", "'")
    return json_text

def dict_to_json_with_start_name(input_dict):
    '''
    Returns a string in json format translates %[0-9] to _[0-9]
    for duplicate tags.
    '''
...
def isMccaAccount(aAccount:str) -> bool:

    rc, value = accountLookup(aAccount)
    if rc == 1:
        found = True
    else:
        found = False

    PrintStr = "isMccaAccount('" + aAccount + "') " + str(found)
    print (PrintStr)
    return found

def CentAmount2DechimalStr(AmountInCents:str) ->str:
   AmountInCents= AmountInCents.zfill(2)
   Amount = AmountInCents[:-2] + '.' + AmountInCents[-2:]
...
if __name__ == '__main__':
   ...
   ExclusiveAmount, VatAmount = CalcInclusiveVatValue(21500, 15.0, 0) #2, float) 
   print (ExclusiveAmount, VatAmount)
   ExclusiveAmount, VatAmount = CalcInclusiveVatValue(21500, 15.5, 0) #2, float) 
   print (ExclusiveAmount, VatAmount)
   print (CentAmount2DechimalStr('0'))
   print (CentAmount2DechimalStr('1'))
   print (CentAmount2DechimalStr('11'))
   print (CentAmount2DechimalStr('111'))
   print (CentAmount2DechimalStr('01111'))
   pass
