...
def runtime_parms():
    ...
    arg_parser.add_argument('-A', '--alias_func_lookup', action='store_true')
    arg_parser.add_argument('-C', '--alias_class_pass',  action='store_true')
    arg_parser.add_argument('-P', '--python_front',      type=str)
...
class Schema:
    ...
    def recurse(self, entry, level=0, depth=0):
        ...
        if tag_type == 'schema':
            ...
            if 'ref' in self.element.attrib:
                logln(f'used as reference {entry.attrib["name"]}')
            else:
                self.group = Group()
                logln(f'used as complexContent {entry.attrib["name"]} perhaps', LogEnum.WARN)
        elif tag_type == 'complexType':
            self.complex = Complex()
            self.complex.prefix = prefix
            def check_types():
            ...
            for key in entry.attrib:
                ...
                elif self.complex != None:
                    self.complex.restriction[key] = entry.attrib[key]
                else:
                    logln('not sure why we have restriction', LogEnum.WARN)
            ...
            if self.content == 'simple':
                ...
                name = entry.attrib['base']
                logln(f'extension name:{name}')
                self.element.extension = entry.attrib['base']
        elif tag_type == 'union':
    ...
    def genaliases(self, no, element):
        if element.level > 0: return
        if element.has('complexType'): return
        attrib = element.attrib
        if 'name' in attrib and 'type' in attrib:
            logln(f'alias {element.attrib["name"]} {element.attrib["type"]}')
            self.aliases.append(attrib)
    ...
    def genclass(self, level):
        ...
        if name in self.complex_map:
            ...
            if len(elements) == 0 and len(attribs) == 0:
                return
            fields = []
            if main.has('complexContent') and main.has('extension'):
                _, extend = main.extension.split(':')
                if extend in self.complex_map:
                    extend_index = self.complex_map[extend]
                    extend = self.elements[extend_index]
                    if extend.has('attrib_fields'):
                        for attrib in extend.attrib_fields:
                            attribs.append(attrib)
                    level = extend.level
                    position = extend.position + 1
                    logln(f'{main.extension}:({level}, {position})')
    ...
    def do_depends(self, name, depth=0):
        if not name in self.classes_depends:
            return
        if depth > 10:
            #logln(f'{name} at depth {depth}', LogEnum.WARN)
            return
        ...
        for depend in depends:
            #logln(f'{depend}')
            self.do_depends(depend, depth+1)
            if depend in self.done:
                continue
            if depend in self.classes:
                class_lines = self.classes[depend]
                for line, indent in class_lines:
                    outline(line, indent)
                if len(class_lines) == 1:
                    outline('pass', indent+1)
    ...
    def genpython(self):
        ...
        if len(self.aliases) > 0:
            outline(f'alias_name = []')
            outline(f'alias_type = []')
            alias_funcs = ''' 
...
'''.splitlines()
            for alias in self.aliases:
                ...
                x = alias_type.split(':')
                if alias_name != x[-1]:
                    outline(f"alias_name.append('{alias_name}')")
                    outline(f"alias_type.append('{x[-1]}')")
            for line in alias_funcs:
                outline(line)
        self.gensimples()
        if self.elements != {}:
            self.elements_dict = {}
