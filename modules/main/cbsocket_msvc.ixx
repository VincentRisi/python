...
public:
	...
	CBSockClient(const char* aName)
	{
		...
		{
			string err = format("{} {} coded:{} format invalid {} {}",
				__FILE__, __LINE__,
				aName,
				SockError(fSockCB), SockErrorMsg(fSockCB));
		...
		if (wError)
		{
			string err = format("{} {} host:{} service:{} invalid {} {}",
				__FILE__, __LINE__,
				names[1], names[2],
				SockError(fSockCB), SockErrorMsg(fSockCB));
			throw XSocketErr(wError, err);
	...
	CBSockClient(const int iSocket, const int Retries = 5, const int TimeOut = 10000)
	{
		...
		if (wError)
		{
			string err = format("{} {} socket:{} retries:{} timeout:{} invalid {} {}",
				__FILE__, __LINE__,
	...
	CBSockClient(const char* aHost, const char* aService, const char* CertFile, const char* CAfile, const char* CApath, const char* KeyFile, const char* KeyPassword)
	{
		...
		{
			fSockCB->CommsPort = -1;
			string err = format("{} {} host:{} service:{} SSL invalid {} {}",
				__FILE__, __LINE__,
				aHost, aService,
				SockError(fSockCB), SockErrorMsg(fSockCB));
			throw XSocketErr(wError, err);
	...
	void Open()
	{
		...
		{
			fSockCB->CommsPort = -1;
			throw_simple_error(wError, format("{} {} Open", __FILE__, __LINE__));
		}
	}
	// Close the socket.
	void  Close()
	{
		...
		if (wError)
			throw_simple_error(wError, format("{} {} Close", __FILE__, __LINE__));
	}
	...
	unsigned int ReadLength()
	{
		...
		if (wError)
			throw_simple_error(wError, format("{} {} ReadLength", __FILE__, __LINE__));
		return wResult;
	}
	...
	uint ReadStream(void* aBuffer, const uint aLength)
	{
		...
		if (wError)
			throw_simple_error(wError, format("{} {} ReadStream", __FILE__, __LINE__));
		return wActual;
	}
	...
	void   WriteLength(const uint aLength)
	{
		...
		if (wError)
			throw_simple_error(wError, format("{} {} WriteLength", __FILE__, __LINE__));
	}
	...
	void   Write(void* aBuffer, const uint aLength)
	{
		...
		if (wError)
			throw_simple_error(wError, format("{} {} Write", __FILE__, __LINE__));
	}
	...
	bool   WaitRead(const uint aTimeout)
	{
		...
		if (wError)
			throw_simple_error(wError, format("{} {} WaitRead", __FILE__, __LINE__));
		return wResult;
	}
	...
	bool   WaitWrite(const uint aTimeout)
	{
		...
		if (wError)
			throw_simple_error(wError, format("{} {} WaitWrite", __FILE__, __LINE__));
		return wResult;
	}
