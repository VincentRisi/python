...
public class OciCMod extends Generator
{
  ...
  static private void generateTable(Table table, String output) throws Exception
  {
    try (PrintWriter outData = new PrintWriter(new FileOutputStream(fileName(output, table.useName().toLowerCase(), ".cppm"))))
    {
      ...
      writeln(format("export module %s;", table.useName().toLowerCase()));
      writeln("import machine;");
      writeln("import ociapi;");
      writeln("import databuilder;");
      writeln("#define JP_MARK __FILE__, __LINE__");
      writeln();
