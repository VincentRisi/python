...
public class MSSqlPyCode extends Generator
{
  private static PrintWriter outLog;
  static private Properties properties;
  static private byte paramStyle = PlaceHolder.PYFORMAT;
  static private byte dbVendor = PlaceHolder.MSSQL;
  ...
  static private void generateStructs(Database database, Table table, String output) throws Exception
  {
    ...
    try (OutputStream outFile = new FileOutputStream(output + dbapiTableName + "DBApi.py"))
    {
      ...
      writeln();
      writeln("import dbapi_util_mssql as dbapi_util");
  ...
  static private void generateCode(Table table)
  {
    for (int i = 0; i < table.procs.size(); i++)
    {
      ...
      if (pairs.size() > 0)
      {
        if (paramStyle == PlaceHolder.PYFORMAT)
        {
          ...
          for (int j = 0; j < pairs.size(); j++)
          {
            ...
            write(3, format("'%1$s': self.%1$s", field.useName()));
          }
          writeln("})");
        }
        else
  ...
  static private void generateCommand(Proc proc, String name, PlaceHolder holder)
  {
    ...
    for (int i = 0; i < lines.size(); i++)
    {
      ...
      else if (proc.isMerge == false || upsert == false)
      {
