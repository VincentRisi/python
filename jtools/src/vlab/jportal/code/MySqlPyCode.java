...
public class MySqlPyCode extends Generator
{
  ...
  static private void generateStructs(Database database, Table table, String output) throws Exception
  {
    ...
    try (OutputStream outFile = new FileOutputStream(output + table.useName() + "DBApi.py"))
    {
      ...
      if (table.hasSequence)
      {
        ...
        writeln(2, "return ''");
        writeln("dbapi_util.returning = MySqlReturning");
        writeln();
        ...
        writeln(1, "def __init__(self, table):");
        ...
        writeln(2, format("_command = f'select max(%s)+1 from {self.table}'", fieldName));
        writeln(2, format("## %s may have to be put in as a parameter", fieldName));
        writeln(2, "## - a more complex generation - rather use returning option");
        ...
        writeln(1, "query = util_sequence(table)");
