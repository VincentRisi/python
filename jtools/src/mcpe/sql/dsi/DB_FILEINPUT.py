...
class DBFileInput(object):
  ...
  def execInsert(self):
    result = self._connect.action('FileInputInsert', self._data())
    self._store(result)
  def runInsert(self, ID, Name, FileDate, FileSize, Message, Header, Trailer, Error, Status, USId, TMstamp):
    ...
    self.execInsert()
  def execUpdate(self):
    result = self._connect.action('FileInputUpdate', self._data())
    self._store(result)
  ...
  def execSelectOne(self):
    result = self._connect.action('FileInputSelectOne', self._data())
    self._store(result)
  ...
  def execSelectOneUpd(self):
    result = self._connect.action('FileInputSelectOneUpd', self._data())
    self._store(result)
  ...
  def querySelectAll(self):
    self._query = self._connect.query('FileInputSelectAll', self._data())
  def fetchSelectAll(self):
    rc, result = self._connect.fetch(self._query)
    record = DBFileInput(self._connect)
    if rc == 1:
...
class DBFileInputDeleteOne(object):
  ...
  def execDeleteOne(self):
    result = self._connect.action('FileInputDeleteOne', self._data())
    self._store(result)
  def runDeleteOne(self, ID):
    self.ID = ID
    self.execDeleteOne()

class DBFileInputExists(object):
  ...
  def execExists(self):
    result = self._connect.action('FileInputExists', self._data())
    self._store(result)
...
class DBFileInputProcessed(object):
  ...
  def execProcessed(self):
    result = self._connect.action('FileInputProcessed', self._data())
    self._store(result)
