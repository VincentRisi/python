...
"user, _ = config['SqlLogonString'].split('/')\n"
"password, server = _.split('@')\n"
"binfile = config['SqlapiBinFile']\n"
"print (user, password, server)\n"
"logon(binfile, user, password, server)\n"
;
...
Putty3Shell::Putty3Shell(const char* name, const char* configfile, const char* prefix)
{
    modulePrefix = strdup(prefix);
    char work[8192];
    snprintf(work, sizeof(work), initial, prefix, prefix, configfile);
    printf("%s\n", work);
