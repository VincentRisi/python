...
function (dbportal2 projectName binFilename siFiles)
  ...
  foreach (siFile ${siFiles})
    ...
    endforeach()
    message (STATUS "--info-- dbportal2() POCIOCI ${pocioci}")
    message (STATUS "--info-- dbportal2() OUTPUT  ${logFile} ${soFile}")
    message (STATUS "--info-- dbportal2() DEPENDS ${siFile}")
    add_custom_command (
...
function (dbportal projectName binFilename siFiles)
  ...
  foreach (siFile ${siFiles})
    ...
    endforeach()
    message (STATUS "--info-- dbportal()POCIOCI ${pocioci}")
    message (STATUS "--info-- dbportal()OUTPUT  ${oFiles}")
    message (STATUS "--info-- dbportal()DEPENDS ${siFile}")    
    add_custom_command (
      OUTPUT  ${oFiles}
...
function (cobbler projectName cobFiles)
  set (outFiles)
  foreach (cobFile ${cobFiles})
    get_filename_component (cobFileDir ${cobFile} DIRECTORY)
    get_filename_component (cobFileName ${cobFile} NAME)
    string (TOLOWER ${cobFileName} cobLowerName)
    set (header ${cobFileDir}/${cobLowerName}.h)
    list (APPEND outFiles ${header})
    add_custom_command (
      OUTPUT  ${header}
      COMMAND java -jar ${cobblerJar} -o ${cobFileDir} -t ${cobFileDir} ${cobFile}
      DEPENDS ${cobFile}
    )
  endforeach ()
  add_custom_target (${projectName} ALL
