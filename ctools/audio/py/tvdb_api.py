...
class Tvdb:
    ...
    def search(self, series):
        ...
        for series in seriesEt:
            result = x = dict((k.tag.lower(), k.text) for k in series.getchildren())
            #print ('703 result;', x)
            result['id'] = x = int(result['id'])
            #print ("705 result['id'];", x, self.config)
            result['lid'] = self.config['langabbv_to_id']['en']#result['language']]
