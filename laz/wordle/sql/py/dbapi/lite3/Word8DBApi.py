...
class DWord8DictList():
    word: Char(9)
    def _make(self): return DWord8DictList()
    __slots__ = ['word']
    def __init__(self):
        self.word = ''
    def _fields(self):
        return DWord8DictList.__slots__
...
class Word8ListByStatus(DWord8):
    def _get_output(self, _result):
        self.word = _result[0]
        self.status = _result[1]
        return 2
...
'''
        ...
        records = []
        for row in cursor:
            record = Word8ListByStatus()
            record._get_output(row)
            records.append(record)
        return records

class Word8DictList(DWord8DictList):
    def _get_output(self, _result):
        self.word = _result[0]
        return 1
